        ...
        async function handleLogin(e) {
            e.preventDefault();
            const username = (document.getElementById('adminUsername') ? document.getElementById('adminUsername').value.trim() : '') || 'admin';
            ...
            try {
                ...
                sessionStorage.setItem('bsp_session', JSON.stringify({ role: 'admin', username: adminUser }));
        ...
        function applyTemplateFromUI() {
            ...
            if (preset.theme) {
                ...
                // Detect light vs dark from the preset's background colour so light
                // templates (e.g. beauty salon with dark:'#FFFFFF') are not overridden.
                const presetDark = (preset.theme.dark || '').replace(/\s/g,'').toLowerCase();
                const isLightPreset = preset.theme.mode === 'light' ||
                    presetDark === '#ffffff' || presetDark === '#fff' ||
                    presetDark === '#f2f2f7' || presetDark === '#fafafa';
                if (isLightPreset) {
                    SITE_CONFIG.theme.mode = 'light';
                    // Light backgrounds already supplied by the preset; keep them.
                } else {
                    SITE_CONFIG.theme.mode = 'dark';
                    // Fill in dark backgrounds if the preset didn't supply them.
                    if (!preset.theme.dark)          SITE_CONFIG.theme.dark          = '#0A0A0A';
                    if (!preset.theme.card)          SITE_CONFIG.theme.card          = '#141414';
                    if (!preset.theme.scrollbarTrack) SITE_CONFIG.theme.scrollbarTrack = '#1C1C1E';
                }
                const modeEl = document.getElementById('themeMode');
                if (modeEl) modeEl.value = SITE_CONFIG.theme.mode;
                applyThemeToPage();
            }
        ...
        window.addEventListener('load', () => {
            loadConfig();
            const _bspSess = JSON.parse(sessionStorage.getItem('bsp_session') || 'null');
            if (sessionStorage.getItem('admin_token') || (_bspSess && _bspSess.role === 'admin')) {
