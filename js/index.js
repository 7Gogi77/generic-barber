            ...
            try {
                if (window.CloudSync && typeof window.CloudSync.loadFromCloud === 'function') {
                    ...
                    if (sdkLoaded) {
                        ...
                        try {
                            if (window.CloudSync && typeof window.CloudSync.loadBookingSettings === 'function') {
                                await window.CloudSync.loadBookingSettings();
                            } else {
        ...
        function renderSiteContent() {
            ...
            if (tableBody) {
                ...
                for (let i = 1; i <= 6; i++) {
                    ...
                    row.innerHTML = `
                        <div class="bh-cell">${dayNamesFull[i]}</div>
                        <div class="bh-cell">${timeText}</div>
                    `;
                    tableBody.appendChild(row);
                }
                const sundayIndex = 0;
                const isSundayWorking = workingDays[sundayIndex] || !daysClosed.includes(sundayIndex);
                const sundayHours = perDayHours[sundayIndex] || {};
                const sundayStart = (sundayHours.start ?? businessHours.start) ?? 9;
                const sundayEnd = (sundayHours.end ?? businessHours.end) ?? 19;
                const sundayText = isSundayWorking ? `${sundayStart}:00 - ${sundayEnd}:00` : 'Zaprto';
                const sundayRow = document.createElement('div');
                sundayRow.className = 'bh-row';
                sundayRow.innerHTML = `
                    <div class="bh-cell">${dayNamesFull[sundayIndex]}</div>
                    <div class="bh-cell">${sundayText}</div>
                `;
                tableBody.appendChild(sundayRow);
            }
        }
        ...
        async function initSite() {
            ...
            // Always fetch bookingSettings fresh from Firebase REST (bypasses CloudSync timing)
            try {
                const _bsR = await fetch('https://barber-shop-9b2ac-default-rtdb.europe-west1.firebasedatabase.app/bookingSettings.json?_t=' + Date.now(), { cache: 'no-store' });
                if (_bsR.ok) {
                    const _bsD = await _bsR.json();
                    if (_bsD && typeof _bsD === 'object') localStorage.setItem('bookingSettings', JSON.stringify(_bsD));
                }
            } catch(_) {}
            // Also check if bookingSettings was embedded inside site_config
            try {
